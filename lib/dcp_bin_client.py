    ...
    def stream_req(self, vbucket, takeover, start_seqno, end_seqno,
        ...
        def __generator(response):
            ...
            while True:
                ...
                if response and response['opcode'] == CMD_DCP_STREAM_END:
                    break
    ...
    def recv_op(self, op):
        ...
        while True:
            try:
                ...
                elif opcode == CMD_DCP_FLOW_CONTROL:
                    # TODO: handle
                    continue

                elif opcode == CMD_DCP_STREAM_REQ:
...
class DcpStream(object):
    ...
    def next_response(self):
        ...
        if response:
            ...
            if response['opcode'] in (CMD_DCP_MUTATION, CMD_DCP_DELETE):
            ...
            if response['opcode'] == CMD_DCP_END_STREAM:
                self._ended = True
...
class OpenConsumer(Open):
    ...
    def __init__(self, name):
        Open.__init__(self, name, FLAG_DCP_OPEN_CONSUMER)

class OpenProducer(Open):
    """ Open producer spec """
    def __init__(self, name, flags, json):
        Open.__init__(self, name, FLAG_DCP_OPEN_PRODUCER | flags, json)

class OpenNotifier(Open):
    """ Open notifier spec """
    def __init__(self, name):
        Open.__init__(self, name, FLAG_DCP_OPEN_NOTIFIER)

class CloseStream(Operation):
    """ CloseStream spec """

    def __init__(self, vbucket, takeover = 0):
        opcode = CMD_DCP_CLOSE_STREAM
...
class StreamRequest(Operation):
    ...
    def formated_response(self, opcode, keylen, extlen, status, cas, body, opaque):
        ...
        elif opcode == CMD_DCP_STREAM_END:
            flags = struct.unpack(">I", body[0:4])[0]
            response = { 'opcode'  : opcode,
                         'vbucket' : status,
                         'flags'   : flags }

        elif opcode == CMD_DCP_MUTATION:
        ...
        elif opcode == CMD_DCP_SNAPSHOT_MARKER:
...
class GetFailoverLog(Operation):
    ...
    def __init__(self, vbucket):
        opcode = CMD_DCP_GET_FAILOVER_LOG
        Operation.__init__(self, opcode,
                           vbucket = vbucket)
