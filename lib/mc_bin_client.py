...
from couchbaseConstants import SET_PKT_FMT, DEL_PKT_FMT, INCRDECR_RES_FMT
from couchbaseConstants import TOUCH_PKT_FMT, GAT_PKT_FMT, GETL_PKT_FMT
...
class MemcachedClient(object):
    ...
    def send_set(self, key, exp, flags, val, vbucket= -1):
        ...
        self._set_vbucket(key, vbucket)
        opaque = self.r.randint(0, 2 ** 32)
        self._sendCmd(couchbaseConstants.CMD_SET, key, val, opaque, struct.pack(SET_PKT_FMT, flags, exp), 0)

    def set_with_meta(self, key, exp, flags, val, dt, vbucket= -1):
        """Set a value in the memcached server without handling the response"""
        self._set_vbucket(key, vbucket)
        opaque = self.r.randint(0, 2 ** 32)
        cas = 10000
        seq = 0
        self._sendMsg(couchbaseConstants.CMD_SET_WITH_META, key, val, opaque, extraHeader=struct.pack("QQII", flags, exp, cas, seq), cas=0, dtype=dt, vbucketId=vbucket)
        return self._handleSingleResponse(opaque)
    ...
    def send_snapshot_marker(self, start, end, vbucket= -1):
        ...
        #self._set_vbucket(key, vbucket)
        opaque = self.r.randint(0, 2 ** 32)
        self._sendCmd(couchbaseConstants.CMD_SNAPSHOT_MARKER, '', '', opaque, struct.pack("QQI", start, end, 1), 0)
        return self._handleSingleResponse(opaque)
    ...
    def send_get(self, key, vbucket= -1):
        ...
        self._set_vbucket(key, vbucket)
        opaque = self.r.randint(0, 2 ** 32)
        self._sendCmd(couchbaseConstants.CMD_GET, key, '', opaque)
    ...
    def create_checkpoint(self, vbucket= -1):
        return self._doCmd(couchbaseConstants.CMD_CREATE_CHECKPOINT, '', '')

    def compact_db(self, key, vbucket, purge_before_ts, purge_before_seq, drop_deletes):
        self._set_vbucket(key, vbucket)
        self.vbucketId = 0
        # some filler bytes were needed
        return self._doCmd(couchbaseConstants.CMD_COMPACT_DB, '', '',
    ...
    def set_flush_param(self, key, val):
        ...
        return self._doCmd(couchbaseConstants.CMD_SET_FLUSH_PARAM, key, val)

    def set_param(self, key, val, type):
        print "setting param:", key, val
        type = struct.pack(couchbaseConstants.GET_RES_FMT, type)
        return self._doCmd(couchbaseConstants.CMD_SET_FLUSH_PARAM, key, val, type)

    def start_onlineupdate(self):
        return self._doCmd(couchbaseConstants.CMD_START_ONLINEUPDATE, '', '')

    def complete_onlineupdate(self):
        return self._doCmd(couchbaseConstants.CMD_COMPLETE_ONLINEUPDATE, '', '')

    def revert_onlineupdate(self):
        return self._doCmd(couchbaseConstants.CMD_REVERT_ONLINEUPDATE, '', '')

    def set_tap_param(self, key, val):
        print "setting tap param:", key, val
        return self._doCmd(couchbaseConstants.CMD_SET_TAP_PARAM, key, val)
    ...
    def get_vbucket_all_vbucket_seqnos(self):
        return self._doCmd(couchbaseConstants.CMD_GET_ALL_VB_SEQNOS,  '', '')
    ...
    def sync_persistence(self, keyspecs):
        ...
        (opaque, cas, data) = self._doCmd(couchbaseConstants.CMD_SYNC, "", payload)
    ...
    def set_time_drift_counter_state(self, vbucket, drift, state):
        """Get the value for a given key within the memcached server."""
        self.vbucketId = vbucket
        extras = struct.pack(couchbaseConstants.SET_DRIFT_COUNTER_STATE_REQ_FMT, drift, state)
        return self._doCmd(couchbaseConstants.CMD_SET_DRIFT_COUNTER_STATE, '', '', extras)
    ...
    def _parse_sync_response(self, data):
        ...
        for i in xrange(nkeys):
            ...
            if eventid == couchbaseConstants.CMD_SYNC_EVENT_PERSISTED:
                spec['event'] = 'persisted'
            elif eventid == couchbaseConstants.CMD_SYNC_EVENT_MODIFED:
                spec['event'] = 'modified'
            elif eventid == couchbaseConstants.CMD_SYNC_EVENT_DELETED:
                spec['event'] = 'deleted'
            elif eventid == couchbaseConstants.CMD_SYNC_EVENT_REPLICATED:
                spec['event'] = 'replicated'
            elif eventid == couchbaseConstants.CMD_SYNC_INVALID_KEY:
                spec['event'] = 'invalid key'
            elif spec['event'] == couchbaseConstants.CMD_SYNC_INVALID_CAS:
    ...
    def get_config(self):
        ...
        return self._doCmd(couchbaseConstants.CMD_GET_CLUSTER_CONFIG, '', '')

    def set_config(self, blob_conf):
        """Set the config within the memcached server."""
        return self._doCmd(couchbaseConstants.CMD_SET_CLUSTER_CONFIG, blob_conf, '')

    def set_collections(self, json):
        return self._doCmd(couchbaseConstants.CMD_COLLECTIONS_SET_MANIFEST, '', json)
